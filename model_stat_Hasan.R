...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#DATA
d<-read.csv("Data/Santruckova2004.csv", sep=',')
...
good_all<-function(xmonod, xmend, xpirt, xdeb){
  ...
  yhat_monod<-yhat_monod[, c("time", "CO2", "CFC14", "G")]
  #Long format
  Yhat_monod<-melt(yhat_monod, id.vars=c("time"))
  #Observations
  Yhat_monod$obs<-c(as.numeric(d$CO214cumul), as.numeric(d$Cmic14),
              as.numeric(d$S))
  ...
  #Selecting measured variables
  yhat_mend<-yhat_mend[, c("time", "CO2", "CFC14", "G")]
  #Long format
  Yhat_mend<-melt(yhat_mend, id.vars=c("time"))
  #Observations
  Yhat_mend$obs<-c(as.numeric(d$CO214cumul), as.numeric(d$Cmic14),
                   as.numeric(d$S))
  ...
  #Selecting measured variables
  yhat_pirt<-yhat_pirt[, c("time", "CO2", "CFC14", "G")]
  #Long format
  Yhat_pirt<-melt(yhat_pirt, id.vars=c("time"))
  #Observations
  Yhat_pirt$obs<-c(as.numeric(d$CO214cumul), as.numeric(d$Cmic14),
                   as.numeric(d$S))
  ...
  yhat_deb<-as.data.frame(ode(y=c(G=d$Sinit[1], e=0, w=w_i, CO2=0),
                              ...
                              times = as.numeric(d$Time)*24))
  #Selecting measured variables
  yhat_deb<-yhat_deb[, c("time", "CO2", "CFC14", "G")]
  #Long format
  Yhat_deb<-melt(yhat_deb, id.vars=c("time"))
  #Observations
  Yhat_deb$obs<-c(as.numeric(d$CO214cumul), as.numeric(d$Cmic14),
                  as.numeric(d$S))
  ...
  stat_all<-data.frame(Variable=c("CO2", "CFC14", "G"),
...
##Monod
monod_par<-as.numeric(read.csv("parameters/hasan_monodpars.csv", header = F))
##Mend
mend_par<-as.numeric(read.csv("parameters/hasan_mendpars.csv", header = F))
##Pirt
pirt_par<-as.numeric(read.csv("parameters/hasan_pirtpars.csv", header = F))
##DEB
deb_par<-as.numeric(read.csv("parameters/hasan_debpars.csv", header = F))

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Models evaluation~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
Hasan_fit<-good_all(monod_par, mend_par, pirt_par, deb_par)
Hasan_fit$Gfit
Hasan_fit$stat_all
Hasan_fit$stat
Hasan_fit$fstat
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Figure~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
Rawdata<-subset(Hasan_fit$Yhat, variable=="CO2" | variable=="CFC14" |
                  variable=="G")
Fits<-subset(Hasan_fit$Yhat_fine, variable=="CO2" |  variable=="CFC14" |
               variable=="G")
