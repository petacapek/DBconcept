...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#DATA
d<-read.csv("Data/Ziegler2005.csv", sep=',')
...
Monod<-function(time, state, pars){
  with(as.list(c(state, pars)),{
    ...
    return(list(c(dB, dG, dCO2), CFC=CFC))
  })
}
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Mend model~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
DEBmodel<-function(time, state, pars){
  with(as.list(c(state, pars)),{
    ...
    return(list(c(dG, de, dw, dCO2), CFC=CFC))
  })
}
...
good_all<-function(xmonod, xmend, xpirt, xdeb){
  ...
  yhat_monod<-yhat_monod[, c("time", "CO2", "CFC", "G")]
  #Long format
  Yhat_monod<-melt(yhat_monod, id.vars=c("time"))
  #Observations
  Yhat_monod$obs<-c(as.numeric(d$CO2), as.numeric(d$Cmic),
              as.numeric(d$S))
  ...
  #Selecting measured variables
  yhat_mend<-yhat_mend[, c("time", "CO2", "CFC", "G")]
  #Long format
  Yhat_mend<-melt(yhat_mend, id.vars=c("time"))
  #Observations
  Yhat_mend$obs<-c(as.numeric(d$CO2), as.numeric(d$Cmic),
                   as.numeric(d$S))
  ...
  #Selecting measured variables
  yhat_pirt<-yhat_pirt[, c("time", "CO2", "CFC", "G")]
  #Long format
  Yhat_pirt<-melt(yhat_pirt, id.vars=c("time"))
  #Observations
  Yhat_pirt$obs<-c(as.numeric(d$CO2), as.numeric(d$Cmic),
                   as.numeric(d$S))
  ...
  yhat_deb<-as.data.frame(ode(y=c(G=d$Sinit[1], e=0, w=w_i, CO2=0),
                              ...
                              times = as.numeric(d$Time)*24))
  #Selecting measured variables
  yhat_deb<-yhat_deb[, c("time", "CO2", "CFC", "G")]
  #Long format
  Yhat_deb<-melt(yhat_deb, id.vars=c("time"))
  #Observations
  Yhat_deb$obs<-c(as.numeric(d$CO2), as.numeric(d$Cmic),
                  as.numeric(d$S))
  ...
  stat_all<-data.frame(Variable=c("CO2", "CFC", "G"),
  ...
  stat_all2<-data.frame(Model=c("Monod", "Mend", "Pirt"),
  ...
  fstatp2<-pf(q=fstat3, df1=(M2p - M1pmend), df2=(nt - M2p), lower.tail=F)
  fstatp3<-pf(q=fstat3, df1=(M2p - M1ppirt), df2=(nt - M2p), lower.tail=F)
  
  stat_all3<-data.frame(Model=c("Monod", "Mend", "Pirt"),
  ...
  good_all_out<-list(Yhat=Yhat_monod, Gfit=Gfit, Yhat_fine = Yhat_all_fine, stat_all=stat_all, stat=stat_all2, fstat=stat_all3)
...
##Monod
monod_par<-as.numeric(read.csv("parameters/ziegler_monodpars.csv", header = F))
##Mend
mend_par<-as.numeric(read.csv("parameters/ziegler_mendpars.csv", header = F))
##Pirt
pirt_par<-as.numeric(read.csv("parameters/ziegler_pirtpars.csv", header = F))
##DEB
deb_par<-as.numeric(read.csv("parameters/ziegler_debpars.csv", header = F))

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Models evaluation~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
Ziegler_fit<-good_all(monod_par, mend_par, pirt_par, deb_par)
Ziegler_fit$Gfit
Ziegler_fit$stat_all
Ziegler_fit$stat
Ziegler_fit$fstat

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Figure~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
Rawdata<-subset(Ziegler_fit$Yhat, variable=="CO2" | variable=="CFC" |
                  variable=="G")
Fits<-subset(Ziegler_fit$Yhat_fine, variable=="CO2" |  variable=="CFC" |
               variable=="G")
